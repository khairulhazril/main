...
Eg. Create an account with username `nicholas` and password `lee`.
...
Eg. Login to an existing account with username `nicholas` and password `lee`.
...
Eg: You are logged into nicholas account and would want to logout.
