...
The Calendar extends the `TaskManager` GUI with an easy to read interface for tracking task deadlines. It is composed of three classes, `CalendarPanel`, `CalendarCell` and `CalendarCellTask`.

`CalendarPanel` is the pane on which the headers and grid of the Calendar are drawn.
`CalendarCell` represents each individual cell of the grid in the `CalendarPanel`.
`CalendarCellTask` represents each individual task inside each `CalendarCell`.
...
The following sequence diagram shows how the Calendar is built on startup:

image::CalendarBuildSequenceDiagram.png[width="1000"]

Whenever the task list is updated, or the selected task is changed, the function `createCalendarCells(taskList)` is called, which replaces the `CalendarCell` and `CalendarCellTask` instances in the `CalendarPanel`.
...
* **Alternative 2:** Have `CalendarCellTask` instances automatically update as the task list changes.
** Pros: No need to rebuild the entire calendar when the task list changes, instead only updating the cell that is affected.
...
. Modify link:{repoURL}blob/master/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete `data/taskmanager.json` so that the application will load the sample data when launched).

===== [Step 6] Storage: Add `Remark` field to the `JsonAdaptedPerson` class
...
.  Should be able to hold up to 250 persons without a noticeable sluggishness in performance for typical usage.
