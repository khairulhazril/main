...
* The login feature is mainly supported by the `Command` class and `login` class. +
There are two types of accounts in login feature which are implemented in the `login` class: +
...
The flow, how the commands are executed and their respective sequence diagrams will be further elaborated below: +
<<Signup Command>> +
<<Login Command>> +
<<Logout Command>> +
<<DeleteAcc Command>>
...
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `data/taskmanager.json` so that the application will load the sample data when you launch it.)
...
|`* *` |user |secure my task manager via a username and password to hide contacts by default |keep my events and due dates private and secure to minimize the chances of someone seeing them by accident
