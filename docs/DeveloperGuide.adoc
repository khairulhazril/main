...
Step 6. If note already exists, _DuplicateNotesException_ will be thrown. This will return a string message "This note already exists in the task manager".
...
Whenever the task list is updated, the selected task is changed, or the month to be displayed changes (see <<Month view change feature,month command>>), the function `createCalendarCells(taskList)` is called, which replaces the `CalendarCell` and `CalendarCellTask` instances in the `CalendarPanel`.
...
Step 1. The user signs in to their account. CommandException(MESSAGE_LOGIN_REQUIRED) will be returned if the user has not signed in before using the command.

Step 2. The user types in `month` with its associated parameter, an integer between 1 and 12 inclusive.
...
If the arguments passes all the checks, the `currMonth` parameter in `Model` will be changed, which will then cause the calendar to be updated.
