...
Step 3. The user executes `add n/Project ...` to add a new task. The `add` command also calls `Model#commitTaskManager()`, causing another modified task manager state to be saved into the `taskManagerStateList`.
...
image::AccountClassDiagram.png[width="450"]
...
Step 1. The user enters the in a note with its associated parameters. e.g `note h/popular c/buy pilot G-2 blue pens p/2`.
...
** Pros: Easy to implement
** Cons: May neglect duplicate notes that mean the same because the check is for the exact same heading and content, co. E.g 1. h/popular c/buy ring file 2.h/popular c/buy ring files
...
Given below is an example usage scenario and how the delete notes mechanism behaves at each step.
...
Step 4. The `LogicManager` will call execute() on the `DeleteNotesCommand` object. If no note of the corresponding index is found, it would return a string of message MESSAGE_INVALID_NOTES_DISPLAYED_INDEX.

Step 5. The `Logic` component then interacts with the Model component which then calls TaskManager component within it to execute deleteNotes(target) to remove the note.

Step 6. The command result would then return the message `MESSAGE_DELETE_NOTE_SUCCESS` in a string.

The following diagram illustrates how the `DeleteNotes` operation works:
...
* **Alternative 1:** Implement a method to strike off notes that are completed so that users can keep track of what notes they have added in and finished.
...
* **Alternative 2(current choice):** Just deleting completed notes away.
** Pros: Easy to implement
...
Final decision: Alternative 2 was chosen due to it being more practical and convenient to users so that users will continue to use it. 
// end::notes[]
...
A project often depends on third-party libraries. For example, Task Manager depends on the https://github.com/FasterXML/jackson[Jackson library] for JSON parsing. Managing these _dependencies_ can be automated using Gradle. For example, Gradle can download the dependencies automatically, which is better than these alternatives:
